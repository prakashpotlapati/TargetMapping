...
def index():
    ...
    if request.method == "POST":
        if "sourceFile" in request.files and "targetFile" in request.files:
            ...
            if source_ext not in allowed_extensions or target_ext not in allowed_extensions:
                flash("Error: Only CSV, XLS, or XLSX files are allowed.", "error")
                return redirect(url_for("index"))
    ...
    try:
        ...
        if os.path.exists(CSV_FILE):
            ...
            if not df.empty:
                # Sort by Weighted Average Score and then filter by Target Column to keep the best matches
                one_to_one_mapping = (
